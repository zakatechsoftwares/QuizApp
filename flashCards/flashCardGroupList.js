...
const GroupListPage = ({ navigation }) => {
  ...
  const removeGroup = async (groupName) => {
    const updatedGroups = groups.filter((group) => Object.keys(group)[0] !== groupName);
    try {
      await AsyncStorage.setItem('cardGroupLists', JSON.stringify(updatedGroups));
    ...
    <View style={styles.container}>
      ...
      {groups !== null ? groups.map((group, index) => (
        <View style={styles.groupItem} key={index}>
          <TouchableOpacity
            onPress={() => navigation.navigate('Flashcards', { groupName: Object.keys(group)[0] })}
          >
            <Text style={[styles.groupName, {flexShrink:1} ]}>{Object.keys(group)[0]}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => navigation.navigate('EditFlashcards',{groupNameEdit: Object.keys(group)[0]})}>
            <Text style={styles.removeButton}>Edit</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => removeGroup(Object.keys(group)[0])}>
