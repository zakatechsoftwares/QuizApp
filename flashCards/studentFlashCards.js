    ...
    <TouchableOpacity onPress={flipCard}>
      <View style={{ alignItems: 'center' }}>
        <Animated.View
          style={[
            {
              transform: [{ rotateY: `${rotateY.value}deg` }], backgroundColor: isFlipped ? 'orange' : 'green', width:'100%', height:'100%'
            },
          ]}
        >
          <Animated.Text style={{ fontSize: 18, transform: [{ rotateY: `${rotateY.value}deg` }], textAlign:'center', textAlignVertical:'center'  }}>
...
const CardViewer = ({navigation,route}) => {
  ...
  return (
    <View style={styles.container} >
      <View style={{height: '80%', width:'100%'}}><FlipCard front={currentCard.front} back={currentCard.back} isFlipped={isFlipped} flipCard={flipCard} rotateY={rotateY}/></View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end',  height:'10%' }}>
        <TouchableOpacity
          onPress={moveToPreviousCard}
          disabled={currentIndex === 0}
          style={{flexDirection:'row', justifyContent:'center', backgroundColor: isFlipped ? 'orange': 'green' }}
        >
